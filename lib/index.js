...
parser.parseString(fs.readFileSync(fileName), function(err, root) {
  ...
  var horizAdvX = font[0].$['horiz-adv-x'];
  var height = fontFace[0].$.ascent - fontFace[0].$.descent;
  ...
  var glyphs = select(font, 'glyph').map(function(glyph) {
    ...
    var w = glyph['horiz-adv-x'] || horizAdvX;
    var h = height;
    var scale = 0.05 * 0.85;
    var matrix = Matrix.I()
        .mult(Matrix.Translate(w * scale * 0.5, h * scale * 0.5, 0))
    ...
    var source = util.format('<svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="%s" height="%s"><path d="%s" fill="%s" /></svg>',
            w * scale, h * scale, conv(glyph.d, matrix), color);
